        ...
        <section className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
          <div className="p-6 border-b border-slate-100 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-400">
              <span className="w-2 h-2 rounded-full bg-emerald-500" /> Auto-syncing live data
            </div>
          </div>
              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {plants.map((plant) => (
                  <tr key={plant.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 font-semibold text-slate-700">{plant.name}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 rounded-md text-[10px] font-bold ${
                        plant.status === "Low Stock" ? "bg-orange-100 text-orange-700" : "bg-emerald-100 text-emerald-700"
                      }`}>
                        {plant.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-slate-500 font-mono">{plant.stock} units</td>
                    <td className="px-6 py-4 text-slate-500">${plant.price}</td>
                    <td className="px-6 py-4 text-emerald-600 font-medium">{plant.growth}</td>
...
function NavItem({ label, active = false }: { label: string; active?: boolean }) {
  return (
    <Link href="#" className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
      active ? "bg-emerald-500/10 text-emerald-400 border-l-4 border-emerald-500" : "text-emerald-100/60 hover:text-white hover:bg-white/5"
    }`}>
      {label}
    </Link>
  );
